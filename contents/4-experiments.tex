...
\WK{Please expand on ResNet18: number of layers (18?), what are the ``bottlenecks'' (a set of three (?) layers, with the smaller one in the middle), etc. You can
include a picture from the resnet paper (with source indicated) if fits to your architecture choice.}
...
\begin{itemize}
    ...
    \item \textbf{BResNet} resemble Plain models, but standard normalization is
        replaced by mean normalization \WK{what is mean normalization?}, so that the first couple layers are
...
\subsection{Degree of equivariance and invariance}
    ...
    \WK{How do you average over the outputs at $k$-th layer (I mean -- the network has a lot of outputs at some particular layer)?}
