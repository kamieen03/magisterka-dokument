...
\WK{At least one good reference is needed in this chapter -- a book (or books) on group theory or some manual from the web}
...
\subsection{Equivariant neural networks}
    ...
    networks which are (roughly) equivariant to translations. The \WK{four} basic
    ...
    the group. \WK{I don't get it: $y \in \mathbb{Z}^2$, so $y \notin G$, right? Should it be a group action on elements of $\mathbb{Z}^2$? Similar problem with the next equation below: now $f_c(y)$ is taken at $y \in G$, while $f_c$ is defined over $\mathbb{Z}^2$. I think there is some inconsistency here\ldots OK, I finally figured it out after reading Cohen et al. :), the first equation applies to the first layer where filters are still defined on $\mathbb{Z}^2$, while in the subsequent layers the filters are defined over $G$. Please expand a bit as it is somewhat misleading.}
